...
import { useImagesGetImages as useImages, useImagesArchiveImage } from '@/__generated__/hooks';
import { format } from 'date-fns';
import { useTranslation } from 'react-i18next';
import type { ImageItem } from '@/app/api/_schemas/images';
...
export function ImageDataTable() {
  ...
  const { data, isLoading, error } = useImages({ query: { page } });
  const archiveMutation = useImagesArchiveImage();
  ...
  const columns: ColumnDef<JobWithParsedParams>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const fileName =
          row.original.params?.originalFileName || row.original.srcImagePath.split('/').pop();
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => handleArchiveClick(row.original.id)}>
  ...
  return (
    <div>
      ...
      <AlertDialog open={isAlertOpen} onOpenChange={setIsAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('imageManagement.dialog.archive.title')}</AlertDialogTitle>
            <AlertDialogDescription>
