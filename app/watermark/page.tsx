...
const WatermarkPage: React.FC = () => {
  ...
  const handleUpdateImageSettings = useCallback(
    (id: string, updatedSettings: Partial<WatermarkSettings>) => {
      setImages(prevImages =>
        prevImages.map(img =>
          img.id === id ? { ...img, settings: { ...img.settings, ...updatedSettings } } : img
        )
      );
    },
    []
      ...
      setImages(prevImages => {
        const newImages = prevImages.map(img => {
          ...
          if (settings.autoIncrement && settings.text !== undefined) {
            const selectedImages = prevImages.filter(i => selectedImageIds.has(i.id));
  ...
  const handleEncode = useCallback(async () => {
    const imagesToEncode = images.filter(
      img => selectedImageIds.has(img.id) && (img.status === 'idle' || img.status === 'error')
    );
    ...
    setImages(prevImages =>
      prevImages.map(img =>
        imagesToEncode.some(encImg => encImg.id === img.id) ? { ...img, status: 'pending' } : img
      )
    );
  ...
  const handleRetryJob = useCallback(
    async (id: string) => {
      ...
      if (image.jobId) {
        ...
        setImages(prevImages =>
          prevImages.map(img => (img.id === id ? { ...img, status: 'idle' } : img))
        );
        toast.info('ジョブをリトライします。再度「透かし埋込を実行」ボタンを押してください。');
      }
    },
    [allImages]
  );
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="fixed bottom-6 right-6">
        <Button onClick={handleEncode} disabled={selectedImageCount === 0} size="lg">
