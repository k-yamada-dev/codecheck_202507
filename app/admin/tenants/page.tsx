...
export default function TenantsPage() {
  ...
  // useAdminGetTenants hookでAPIからテナント一覧を取得
  const { data, isLoading, error, refetch } = useAdminGetTenants();
  const tenants = data?.tenants ?? [];

  // 自動生成hookでCRUD
  const createTenantMutation = useAdminCreateTenant();
  const updateTenantMutation = useAdminUpdateTenant();
  const deleteTenantMutation = useAdminDeleteTenant();
  ...
  const handleFormSubmit = async (values: FormData) => {
    ...
    try {
      ...
      if (editingTenant) {
        ...
        await createTenantMutation.mutateAsync({ body: parsedValues });
      }
  ...
  if (error)
    return (
      <div>
        An error occurred
        <pre style={{ color: 'red', whiteSpace: 'pre-wrap' }}>{JSON.stringify(error, null, 2)}</pre>
      </div>
    );

  // デバッグ用: dataの内容を一時表示
  if (!isLoading && !error && (!data || !Array.isArray(data.tenants))) {
    return (
      <div>
      ...
      <Dialog open={isFormDialogOpen} onOpenChange={setIsFormDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingTenant ? 'Update the tenant details.' : 'Enter details for the new tenant.'}
            </DialogDescription>
          </DialogHeader>
          <TenantForm
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the tenant &quot;{deletingTenant?.name}&quot;. This
