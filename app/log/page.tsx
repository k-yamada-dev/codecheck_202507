    ...
    <div className="container mx-auto p-4">
      <Tabs value={filter} onValueChange={value => setFilter(value as FilterType)}>
        <TabsList className="grid w-full grid-cols-3">
          ...
          <TabsTrigger value="embed">{t('log.tabs.embed', 'Embed')}</TabsTrigger>
          <TabsTrigger value="decode">{t('log.tabs.decode', 'Decode')}</TabsTrigger>
        </TabsList>
      </Tabs>
      ...
      <Card className="my-4">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Input
              placeholder={t('log.filter.search', 'Search Job ID, user, text...')}
              value={searchTerm}
              onChange={e => setSearchTerm(e.target.value)}
            />
            <Input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} />
            <Input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} />
            <Button onClick={() => jobQuery.refetch()} disabled={jobQuery.isFetching}>
              {jobQuery.isFetching ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
      ...
      {jobQuery.isError && (
        <div className="text-center text-red-500">{t('log.error', 'Failed to load logs.')}</div>
      )}
