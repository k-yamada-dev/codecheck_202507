...
export const DecodeForm: React.FC<DecodeFormProps> = ({ onSuccess }) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!uploadResponse.ok) throw new Error(t('decode.errors.uploadFailed'));
      ...
      if (!decodeResponse.ok) throw new Error(t('decode.errors.decodeFailed'));
      const { result } = await decodeResponse.json();
      onSuccess?.(result);
    } catch (error) {
      const message = error instanceof Error ? error.message : t('decode.errors.unknown');
      setError(message);
