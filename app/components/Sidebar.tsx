...
export default function Sidebar() {
  ...
  if (status === 'loading') {
    ...
    return <div className="w-16 md:w-64 bg-gray-100 dark:bg-gray-800"></div>;
  }

  const userRoles = session?.user?.roles || [];
  const menuItems = navItems.filter(item => item.roles.some(role => userRoles.includes(role)));
          ...
          <SheetContent side="left" className="w-56 p-0">
            <div className="flex flex-col h-full">
              <div className="p-4 flex items-center justify-center border-b h-14">
                {/* <h1 className="font-bold text-lg">AcuaSaaS</h1> */}
              </div>
              {navContent}
            </div>
          </SheetContent>
      ...
      >
        <div className="flex flex-col h-full">
          <div className="p-2 flex items-center justify-between border-b h-14">
            <h1 className={cn('font-bold text-lg pl-2', { hidden: isCollapsed })}>AcuaSaaS</h1>
